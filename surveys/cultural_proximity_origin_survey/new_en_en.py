...
parser.add_argument("--n_samples", type=int, required=False, default=1, help="number of survey responses to collect per model")
...
class EmotionElicit:
    ...
    def __init__(self, model_name):
        ...
        self.model = self.__create_model(models[model_name])
    ...
    def __create_prompt(self, i, format_instructions):
        if i == 0:
            return PromptTemplate(template="We value your participation. \
                    ...
                    as a participant you may not feel the same way. Please rate this questionnaire as if you were a participant\
                    from the US.\n{format_instructions}\n{situation}",\
...
# signal.alarm(300) # Set timer limit

new = EmotionElicit(mname)


os.makedirs(f"results/{mname}", exist_ok=True)

logging.basicConfig(filename=mname+"_en.log", encoding='utf-8', level=logging.DEBUG)
for i in range(n_samples):
    # try:
        df = new.generate_response()
    #     signal.alarm(0)  # Set alarm to 0
    # except TimeoutError as e:
    #     print(e)

        folder = f"results/{mname}"
        prefix = f"en_{mname}"
        ext = ".csv"

        # Find the next available i in the folder
        j = 0
        while os.path.exists(os.path.join(folder, f"{prefix}{j}{ext}")):
            j += 1

        # Save the DataFrame
        filename = os.path.join(folder, f"{prefix}{j}{ext}")
