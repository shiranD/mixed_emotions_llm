    ...
    def __create_prompt(self, i, format_instructions):
        if i == 0:
            return PromptTemplate(template="We value your participation. \
                    ...
                    as a participant you may not feel the same way. Please rate it as an American participant.\n{format_instructions}\n{situation}",\
...
if name == ("meta-llama/llama-2-70b-chat"):
    fname = "llama"
logging.basicConfig(filename=fname+"_en_en.log", encoding='utf-8', level=logging.DEBUG)
for i in range(1): # choose n for how many times to run this survey
    df = new.generate_response()
    df.to_csv("results/conditioning/third_results/"+fname+"/en_en_"+fname+str(i))
