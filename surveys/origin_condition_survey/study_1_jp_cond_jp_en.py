        ...
        if i == 0:
            return PromptTemplate(template="ご参加いただきありがとうございます。\
                    ...
                    0（全く感じない）から4（非常に強く感じる）までの5段階の段階評価でお願いします。\
                     どのように感じるかは個人差があります。アメリカの参加者として評価してください。\n{format_instructions}\n{situation}",\
...
logging.basicConfig(filename=fname+"_jp_en.log", encoding='utf-8', level=logging.DEBUG)
for i in range(1): # choose n for how many times to run this survey
    df = new.generate_response()
    df.to_csv("results/conditioning/third_results/"+fname+"/jp_en_"+fname+str(i))
