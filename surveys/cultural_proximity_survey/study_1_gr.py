    ...
    def __create_prompt(self, i, format_instructions):
        ...
        if i == 1:
            return PromptTemplate(template="Wir schätzen Ihre Teilnahme. Die folgende Umfrage befasst sich mit Kultur und Emotionen. \
                    ...
                    Bewertungen von 0 (überhaupt nicht) bis 5 (sehr stark). Während die Situation möglicherweise die Gefühle der Person widerspiegelt, die sie geschrieben hat, empfinden Sie als Teilnehmer möglicherweise nicht die gleichen Gefühle\n{format_instructions}\n{situation}",\
                    input_variables=["situation"],
                    partial_variables={"format_instructions": format_instructions})
        if i == 2:
            return PromptTemplate(template="Wir schätzen Ihre Teilnahme. Die folgende Umfrage befasst sich mit Kultur und Emotionen. \
