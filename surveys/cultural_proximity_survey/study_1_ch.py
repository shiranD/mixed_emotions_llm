...
class EmotionElicit:
    ...
    def __create_prompt(self, i, format_instructions):
        if i == 0:
            return PromptTemplate(template="我们非常重视你的参与。以下调查问卷关于文化和情绪。请根据描述的情境，对你的整体的积极情绪和整体的消极情绪进行评分。\
                    请使用从0（完全没有）到4（非常强烈）的5点量表。下述情境可能表达了调查问卷作者的感受，作为调查问卷的参与者，你可能并不会有相同的感受。\
                    \n{format_instructions}\n{situation}",\
                    input_variables=["situation"],
                    partial_variables={"format_instructions": format_instructions})
        if i == 1:
            return PromptTemplate(template="我们非常重视你的参与。以下调查问卷关于文化和情绪。\
                    请根据描述的情境，对你的特定的积极情绪，如快乐、自豪、同情、解脱、希望、友善的感觉，和特定的消极情绪，如悲伤、焦虑、生气、\
                    自责、害怕、对自己生气、羞耻、内疚、嫉妒、挫败、尴尬、怨恨、担心麻烦别人，进行评分。请使用从0（完全没有）到5（非常强烈）的6点量表。\
                    下述情境可能表达了调查问卷作者的感受，作为调查问卷的参与者，你可能并不会有相同的感受。\n{format_instructions}\n{situation}",\
                    input_variables=["situation"],
                    partial_variables={"format_instructions": format_instructions})
        if i == 2:
            return PromptTemplate(template="我们非常重视你的参与。以下调查问卷关于文化和情绪。\
                    请根据描述的情境，对你愿意为他人的情绪承担责任的程度，你把自己的情绪归责于他人的程度，以及你想要根据自己的愿望影响或改变周围人、\
                    事、物的程度，进行评分。请使用从0（完全没有）到4（非常强烈）的5点量表。下述情境可能表达了调查问卷作者的感受，作为调查问卷的参与者，你可能并不会有相同的感受。\
                    \n{format_instructions}\n{situation}",\
                    input_variables=["situation"],
                    partial_variables={"format_instructions": format_instructions})
