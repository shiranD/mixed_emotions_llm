...
class EmotionElicit:
    ...
    def __create_prompt(self, i, format_instructions):
        if i == 0:
            return PromptTemplate(template="Chúng tôi đánh giá cao sự tham gia của bạn. Sau đây là khảo sát về văn hóa và cảm xúc.\
                     Xin hãy đánh giá tình huống sau dựa theo cảm xúc tiêu cực và cảm xúc tiêu cực của bạn theo thang điểm 5 với nhãn từ 0 (không cảm thấy gì)\
                     đến 4 (cảm thấy sâu sắc). \
                    Trong mỗi tình huống, người viết có bày tỏ cảm xúc của mình, nhưng cảm xúc của bạn có thể khác hoàn toàn.\n{format_instructions}\n{situation}",\
        ...
        if i == 1:
            return PromptTemplate(template="Chúng tôi đánh giá cao sự tham gia của bạn. Sau đây là khảo sát về văn hóa và cảm xúc.\
                    ...
                    theo thang điểm 6 với nhãn từ  0 (không cảm thấy gì) đến 5 (cảm thấy sâu sắc). Trong mỗi tình huống, người viết có bày tỏ cảm xúc của mình,\
                     nhưng cảm xúc của bạn có thể khác hoàn toàn.\n{format_instructions}\n{situation}",\
                    input_variables=["situation"],
                    partial_variables={"format_instructions": format_instructions})
        if i == 2:
            return PromptTemplate(template="Chúng tôi đánh giá cao sự tham gia của bạn. \
                    Sau đây là khảo sát về văn hóa và cảm xúc. \
                    Xin hãy đánh giá tình huống sau dựa mức độ mà bạn chịu trách nhiệm cho cảm xúc của người khác, \
                    mức độ mà người khác chịu trách nhiệm về bạn và, mức độ mà bạn bạn nghĩ về việc ảnh hưởng hoặc thay đổi những người, \
                    sự kiện, đối tượng xung quanh dựa theo ý muốn riêng của bạn, sử dụng thang điểm 5 với nhãn từ 0 (không cảm thấy gì) đến 5 (cảm thấy sâu sắc).\
                    Trong mỗi tình huống, người viết có bày tỏ cảm xúc của mình, nhưng cảm xúc của bạn có thể khác hoàn toàn.\n{format_instructions}\n{situation}",\
                    input_variables=["situation"],
                    partial_variables={"format_instructions": format_instructions})
